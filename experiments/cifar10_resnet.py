...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 12 11:58:51 2021
...
from pbms.models import ResNet, ProbResNet
...
width = [8, 16, 32]
blocks = [[2, 2, 2, 2], [3, 4, 6, 3]]

n_models = len(width) * len(blocks)
prior_weight = 1/22

# Set up for saving results
path = 'RESULTS/CIFAR10_ResNet/'
save = True
...
for b in blocks:
    for w in width:
            ...
            ######## Prior means network ########
            # Train model
            prior = ResNet(image_size, n_classes, w, b).to(device)
            prior_optimizer = optim.SGD(prior.parameters(), lr = 0.01, momentum = 0.9)
            ...
            prior2 = ProbResNet(image_size, n_classes, w, b, sigma_prior, True, prior, device).to(device)
            optimizer = optim.SGD(prior2.parameters(), lr = 0.01, momentum = 0.9)
            obj_progress, loss_progress, prior_var_progress = trainProbNN(prior2, optimizer, 'classic', True, prior_epochs, data.prior_loader, delta, pmin, prior_weight, device, verbose=False)
            ...
            prob = ProbResNet(image_size, n_classes, w, b, sigma_prior, False, prior2, device).to(device)
            optimizer = optim.SGD(prob.parameters(), lr = 0.01, momentum = 0.9)
            obj_progress, loss_progress = trainProbNN(prob, optimizer, 'classic', False, train_epochs, data.train_loader, delta, pmin, prior_weight, device, verbose=False)
            ...
            results.iloc[counter, :] = ['ResNet', depth, w, n_parameters, float(ce_bound), float(error_bound), stch_loss, stch_err, post_loss, post_err, ens_loss, ens_err, train_ens_err, error_prior, train_err_prior]
            
            counter += 1
            
            if save:
                results.to_csv(path + 'results_cifar_resnet.csv')
